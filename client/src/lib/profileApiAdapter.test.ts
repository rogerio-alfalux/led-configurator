...
function makePerfilProduct(overrides: Partial<ApiProduct> = {}): ApiProduct {
  return {
    ...
    produto: "EASY PRIME E IF 1B 582MM",
    moduloLed: "18W STRIPFLEX 350MA 25V",
    otica: "NÃO APLICÁVEL",
    ...
    driverOnoff220: "PHILIPS XITANIUM 19W 350MA",
    driverOnoffBivolt: "LIFUD 13W 350MA BIVOLT",
...
describe("adaptProfileProducts", () => {
  ...
  it("popula módulos ML corretamente", () => {
    const products = [
      makePerfilProduct({
        ...
        produto: "EASY PRIME E ML 1B 570MM",
      }),
    ];
    const result = adaptProfileProducts(products);
    const variant = result!["LLE-2580"];
  ...
  it("popula módulos IN com barras decimais", () => {
    const products = [
      makePerfilProduct({
        ...
        produto: "EASY PRIME E IN 1.4B 839MM",
      }),
    ];
    const result = adaptProfileProducts(products);
    const variant = result!["LLE-2580"];
  ...
  it("agrupa múltiplos módulos do mesmo perfil", () => {
    const products = [
      makePerfilProduct({ sku: "LLE-2580.1IF.18F", produto: "EASY PRIME E IF 1B 582MM" }),
      makePerfilProduct({ sku: "LLE-2580.2IF.18F", produto: "EASY PRIME E IF 2B 1142MM" }),
      makePerfilProduct({ sku: "LLE-2580.1ML.18F", produto: "EASY PRIME E ML 1B 570MM" }),
      makePerfilProduct({ sku: "LLE-2580.1IN.18F", produto: "EASY PRIME E IN 1B 589MM" }),
    ];
    const result = adaptProfileProducts(products);
    const variant = result!["LLE-2580"];
  ...
  it("cria variantes separadas para diferentes instalações do mesmo perfil", () => {
    const products = [
      makePerfilProduct({ sku: "LLE-2810.1IF.18F", produto: "BLAZE E IF 1B 575MM", instalacao: "EMBUTIR", familia: "BLAZE" }),
      makePerfilProduct({ sku: "LLS-3945.1IF.39F", produto: "BLAZE S IF 1B 575MM", instalacao: "SOBREPOR", familia: "BLAZE" }),
    ];
    const result = adaptProfileProducts(products);
  ...
  it("ignora produtos com código de perfil desconhecido", () => {
    const products = [
      makePerfilProduct({ sku: "LLX-9999.1IF.18F", produto: "DESCONHECIDO E IF 1B 500MM" }),
    ];
    const result = adaptProfileProducts(products);
    // Retorna null pois nenhum produto válido foi processado
    expect(result).toBeNull();
  });
  ...
  it("ignora produtos com nome que não segue o padrão IF/ML/IN", () => {
    const products = [
      makePerfilProduct({ produto: "EASY PRIME E ESPECIAL 500MM" }),
    ];
    const result = adaptProfileProducts(products);
    expect(result).toBeNull();
  });
  ...
  it("retorna catálogo com todos os 20 perfis quando dados completos são fornecidos", () => {
    // Simula um produto para cada código de perfil
    const profileCodes = [
