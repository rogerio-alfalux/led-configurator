                ...
                <Card>
                  <CardHeader className="pb-2">
                    <CardTitle className="text-sm flex items-center gap-2">
                      ...
                      <Badge variant="outline" className="text-xs ml-auto text-purple-700 border-purple-300">Notas faturadas</Badge>
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-1">
                    {((managerData as any)?.invoicedByCompany ?? []).length === 0 ? (
                      <p className="text-sm text-muted-foreground text-center py-3">Nenhum faturamento no período</p>
                    ) : (
                      ((managerData as any)?.invoicedByCompany ?? []).map((company: any) => (
                        <div key={company.billingCompany} className="flex items-center justify-between gap-3 px-3 py-2 rounded-lg hover:bg-muted/50">
                          <div className="min-w-0">
                            <p className="text-sm font-semibold truncate">{company.billingCompany}</p>
                            <p className="text-xs text-muted-foreground">{Number(company.count)} nota{Number(company.count) === 1 ? "" : "s"} no período</p>
                          </div>
                          <p className="text-sm font-bold text-purple-600 dark:text-purple-400 tabular-nums">{formatBRL(Number(company.totalAmount ?? 0))}</p>
                        </div>
                      ))
                    )}
                  </CardContent>
