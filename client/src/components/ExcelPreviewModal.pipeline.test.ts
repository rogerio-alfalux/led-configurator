...
describe("pipeline visual do PDF oficial e LD", () => {
  ...
  it("anexa automaticamente ao LD a mesma prévia oficial sem abrir diálogo de arquivo", () => {
    ...
    const officialCloneStyle = previewSource.slice(
      previewSource.indexOf("const officialPrintStyle"),
      previewSource.indexOf("// O tema pode propagar OKLCH"),
    );
    expect(officialCloneStyle).not.toContain("zoom: 1 !important");
    expect(officialCloneStyle).not.toContain("transform: none !important");
