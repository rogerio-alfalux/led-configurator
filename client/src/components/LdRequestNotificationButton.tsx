...
export function LdRequestNotificationButton({ href, title, count, tone }: { href: string; title: string; count: number; tone: "admin" | "guest" }) {
  ...
  return <Link href={href}>
    <Button data-testid="ld-request-access" variant="ghost" size="icon" title={title} className="relative text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground">
      <ClipboardList data-testid="ld-request-icon" className="w-4 h-4" />
