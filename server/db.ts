...
export async function addQuoteRevision(
  ...
  await db.update(quotes).set({
    clientName: input.clientName ?? quote.clientName,
    clientContact: input.clientContact !== undefined ? (input.clientContact ?? null) : quote.clientContact,
    clientPhone: input.clientPhone !== undefined ? (input.clientPhone ?? null) : quote.clientPhone,
    clientEmail: input.clientEmail !== undefined ? (input.clientEmail ?? null) : quote.clientEmail,
    projectName: input.projectName !== undefined ? (input.projectName ?? null) : quote.projectName,
    projectRef: input.projectRef !== undefined ? (input.projectRef ?? null) : quote.projectRef,
    vendorName: input.vendorName !== undefined ? (input.vendorName ?? null) : quote.vendorName,
    assistantName: input.assistantName !== undefined ? (input.assistantName ?? null) : quote.assistantName,
    seller1Id: input.seller1Id !== undefined ? (input.seller1Id ?? null) : quote.seller1Id,
    seller1Name: input.seller1Name !== undefined ? (input.seller1Name ?? null) : quote.seller1Name,
    seller2Id: input.seller2Id !== undefined ? (input.seller2Id ?? null) : quote.seller2Id,
    seller2Name: input.seller2Name !== undefined ? (input.seller2Name ?? null) : quote.seller2Name,
    assistantId: input.assistantId !== undefined ? (input.assistantId ?? null) : quote.assistantId,
    notes: input.notes !== undefined ? (input.notes ?? null) : quote.notes,
    currentVersion: newVersion,
    totalAmount: String(input.totalAmount),
    totalFinal: input.totalFinal != null ? String(input.totalFinal) : String(input.totalAmount),
    rtPercent: input.rtPercent != null ? String(input.rtPercent) : (quote.rtPercent ?? '0'),
    rtDest1: input.rtDest1 !== undefined ? (input.rtDest1 ?? null) : quote.rtDest1,
    rtDest1Active: input.rtDest1Active !== undefined ? input.rtDest1Active : (quote.rtDest1Active ?? false),
    rtDest2: input.rtDest2 !== undefined ? (input.rtDest2 ?? null) : quote.rtDest2,
    rtDest2Active: input.rtDest2Active !== undefined ? input.rtDest2Active : (quote.rtDest2Active ?? false),
    ...
    deliveryDays: input.deliveryDays !== undefined ? input.deliveryDays : (quote.deliveryDays ?? 20),
    commissionPercent: input.commissionPercent != null ? String(input.commissionPercent) : (quote.commissionPercent ?? '0.05'),
    paymentTerm: input.paymentTerm !== undefined ? (input.paymentTerm ?? '30% Sinal e 70% a 28DDF (mediante aprovação de cadastro)') : (quote.paymentTerm ?? '30% Sinal e 70% a 28DDF (mediante aprovação de cadastro)'),
    destState: input.destState !== undefined ? (input.destState ?? null) : quote.destState,
    difalEnabled: input.difalEnabled !== undefined ? input.difalEnabled : (quote.difalEnabled ?? false),
    difalPercent: input.difalPercent != null ? String(input.difalPercent) : (quote.difalPercent ?? '0'),
    fcpPercent: input.fcpPercent != null ? String(input.fcpPercent) : (quote.fcpPercent ?? '0'),
...
/**
 * Verifica se um e-mail é de um assistente externo (exceção de domínio).
 * Esses usuários devem ter o role 'assistente' atribuído automaticamente.
 */
...
export async function getDashboardProductAnalytics(year: number, month?: number, dateFrom?: string, dateTo?: string) {
  ...
  const activityQuotes = await db.select({
    ...
    openInPeriod: sql<number>`CASE WHEN ${quotes.status} = 'open' AND ${createdInPeriod} THEN 1 ELSE 0 END`,
  ...
  const normalizedQuotes = activityQuotes.map((quote) => ({
    ...
    openInPeriod: Boolean(quote.openInPeriod),
