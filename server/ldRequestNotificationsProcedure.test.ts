...
describe("notificações de solicitações LD", () => {
  ...
  it("retorna e baixa individualmente respostas do LD após download", async () => {
    ...
    await expect(caller.ldRequests.markResponseViewed({ requestId: 12 })).resolves.toEqual({ success: true });
