  ...
  quotes: router({
    save: commercialQuoteProcedure
      .input(z.object({
        ...
        difalFcpIncluded: z.boolean().optional(),
    ...
    updateStatus: commercialQuoteProcedure
      .input(z.object({
        ...
        approvedDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Data de aprovação inválida").optional(),
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (input.approvedDate && input.status !== "approved") {
          throw new TRPCError({ code: "BAD_REQUEST", message: "A data de aprovação só pode ser informada ao aprovar o orçamento." });
        }
    ...
    appendItems: commercialQuoteProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const result = await addQuoteRevision(input.quoteId, {
          ...
          difalFcpIncluded: quote.difalFcpIncluded ?? false,
